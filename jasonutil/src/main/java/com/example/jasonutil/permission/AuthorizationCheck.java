...
public class AuthorizationCheck {
     ...
     * @param authorizationPrompt 权限名字
     * @return true 有权限
     */
    public static boolean authorizationPermission(Activity activity, String checkAuthorization, String authorizationPrompt) {
        boolean isAuthorization = true;
        //1.检测权限
        if (!authorizationCheck(checkAuthorization, activity)) {
            //获取上次被拒权限列表
            ...
            if (isRefuse) {
                authorizationPrompt(activity, authorizationPrompt, true, true);
            } else {
                //2.没有权限，弹出对话框申请
    ...
    public static boolean authorizationPermission(Activity activity, String[] checkAuthorization, String[] authorizationPrompt) {
        ...
        for (int i=0;i< checkAuthorization.length;i++) {
            ...
            if (!authorizationCheck(checkAuthorization[i], activity)) {
                ...
                if (isRefuse) {
                    authorizationPrompt(activity, authorizationPrompt[i], true, true);
                } else {
                    //2.没有权限，弹出对话框申请
                    ActivityCompat.requestPermissions(activity, checkAuthorization, REQUEST_CODE);
                }
     ...
     * @return true 有权限
     */
    public static boolean authorizationPermission(RiggerPresenter presenter,Activity activity,Fragment fragment, String[] checkAuthorization, String[] authorizationPrompt) {
        boolean isAuthorization = true;
        for (int i=0;i< checkAuthorization.length;i++) {
            //1.检测权限
            if (!authorizationCheck(checkAuthorization[i], activity)) {
     ...
     * @return true 有权限
     */
    public static boolean authorizationPermission(RiggerPresenter presenter,Activity activity, android.app.Fragment fragment, String[] checkAuthorization, String[] authorizationPrompt) {
        boolean isAuthorization = true;
        for (int i=0;i< checkAuthorization.length;i++) {
            //1.检测权限
            if (!authorizationCheck(checkAuthorization[i], activity)) {
     ...
     */
    public static void authorizationPrompt(final Activity activity, String authorizationPrompt,
                                           boolean withDialog, final boolean isMustOpen) {
        String promptWithRead = activity.getString(R.string.not_obtainable1) + authorizationPrompt + activity.getString(R.string.open_access_path1);
        if (!withDialog) {
            if (isMustOpen) {
                ToastShow.showToastShowCenter(activity, promptWithRead);
