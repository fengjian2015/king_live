...
public class GiftController {
     ...
     * 礼物队列
     */
    private List<AbstractGiftModel> mGiftQueue = new ArrayList<>();
    /**
     * 礼物控件集合
     */
     ...
     * @return
     */
    public GiftController append(GiftAnimationLayout layout, AbstractGiftViewHolder holder, ICustomerAnimation animation, boolean hideMode) {
     ...
     * 加入礼物
     */
    public void addGift(AbstractGiftModel giftModel) {
     ...
     */
    public void addGift(AbstractGiftModel giftModel, boolean supportCombo) {
        if (supportCombo) {
            for (int i = 0; i < mGiftLayouts.size(); i++) {
                ...
                AbstractGiftModel currentModel = layout.getCurrentGiftModel();
     ...
     * 添加一个礼物到礼物队列中
     */
    private void addGiftToQueue(final AbstractGiftModel giftModel, final boolean supportCombo) {
        ...
        if (supportCombo) {
            ...
            for (AbstractGiftModel model : mGiftQueue) {
     ...
     * 取出队列中的下一个礼物
     */
    private synchronized AbstractGiftModel getNextGiftModel() {
        AbstractGiftModel giftModel = null;
        if (mGiftQueue.size() != 0) {
