...
public class GiftAnimationLayout extends FrameLayout {
     ...
     * 礼物停止连击动画后继续展示的时间
     */
    private int giftDismissTime = 2000;
     ...
     * 设置视图
     */
    public void setViewHolder(AbstractGiftViewHolder holder) {
        this.holder = holder;
     ...
     * 更新礼物信息
     */
    public synchronized void updateGift(AbstractGiftModel giftModel) {
        //增加礼物数量，用于连击效果
     ...
     * 加载礼物到当前控件
     */
    public boolean loadGift(AbstractGiftModel giftModel) {
        if (giftModel == null) {
            return false;
        }
        mGiftModel = giftModel;
        giftDismissTime=mGiftModel.getGif_time();
    ...
    public void singleComboAnimationEnd() {
        ...
        //比较当前连击数与礼物总数，大于的情况下继续播放连击动画，否则延迟giftDismissTime后通知播放退出动画
        if (mComboTotalCount > mComboCurrentCount) {
            ...
            mHandler.postDelayed(mGiftExitAnimationRunnable, giftDismissTime);
            startCheckGiftCountRunnable();
        }
    }

    /**
     * 开启新的检查runnable，用于检查{@link #giftDismissTime}时间内连击数是否有改变
     */
